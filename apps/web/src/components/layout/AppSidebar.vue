...
<template>
  <!-- Desktop Sidebar -->
  <aside class="hidden md:flex flex-col h-screen w-64 bg-surface-container-low dark:bg-surface-dark p-6 fixed left-0 top-0 z-40 border-r soft-brutalist-border">
    <!-- Brand Logo Header -->
    <div class="mb-8 select-none">
      <h1 class="text-xl font-bold text-on-surface dark:text-surface-container-lowest font-serif leading-none tracking-tight">Vincis</h1>
      <p class="text-[10px] font-sans font-bold uppercase tracking-widest text-on-surface-muted mt-1.5">Academic Portal</p>
    </div>
    ...
    <nav class="flex-1 space-y-3">
      <router-link
        ...
        :key="item.id"
        :to="item.path"
        class="flex items-center gap-3 px-4 min-h-[48px] rounded-xl transition-all"
      ...
      >
        <i
          class="pi text-[20px]"
          :class="item.icon"
          :style="isActive(item.path) ? { textShadow: '0 0 1px currentColor' } : {}"
        ></i>
        <span
          class="text-sm tracking-wide font-sans"
